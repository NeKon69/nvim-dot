return {
	{
		...
		config = function()
			...
			local function append_dap_log(event_name, payload)
				...
				local line =
					string.format("[%s] %s %s", os.date("%Y-%m-%d %H:%M:%S"), event_name, safe_json(payload or {}))
			...
			local function get_codelldb_adapter_command()
			...
			local codelldb_command = get_codelldb_adapter_command()
			append_dap_log("dap_codelldb_adapter_configured", {
				command = codelldb_command,
			})
			if codelldb_command then
			...
			local pre_debug_jobs = {}

			local function stop_pre_debug_jobs()
				for job_id, _ in pairs(pre_debug_jobs) do
					pcall(vim.fn.jobstop, job_id)
					pre_debug_jobs[job_id] = nil
				end
			end

			local function run_dap_spec(spec)
				local launch_spec = vim.deepcopy(spec or {})
				local build_task = launch_spec.build_task
				launch_spec.build_task = nil

				if type(build_task) == "string" and build_task ~= "" then
					local overseer = require("overseer")
					overseer.run_task({
						name = build_task,
						params = { profile = _G.BuildSystem and _G.BuildSystem.profile or nil },
					}, function(task)
						if not task then
							vim.notify("Build task not found: " .. build_task, vim.log.levels.ERROR)
							return
						end
						task:subscribe("on_complete", function(_, status)
							if status == "SUCCESS" then
								run_dap_spec(launch_spec)
							else
				...
				local pre_debug = launch_spec.pre_debug
				launch_spec.pre_debug = nil

				if type(pre_debug) ~= "table" then
					dap.run(launch_spec)
					return
				end

				local cwd = pre_debug.cwd or launch_spec.cwd or vim.fn.getcwd()
				local cmd = pre_debug.cmd or pre_debug.command
				if cmd == nil and type(pre_debug.script) == "string" and pre_debug.script ~= "" then
					local script = pre_debug.script
					if not script:match("^/") and not script:match("^[A-Za-z]:[\\/]") then
						script = cwd .. "/" .. script
					end

					cmd = {}
					if type(pre_debug.interpreter) == "string" and pre_debug.interpreter ~= "" then
						table.insert(cmd, pre_debug.interpreter)
					end
					table.insert(cmd, script)
					for _, arg in ipairs(pre_debug.args or {}) do
						table.insert(cmd, tostring(arg))
					end
				end
				if type(cmd) ~= "string" and type(cmd) ~= "table" then
					vim.notify("pre_debug is missing 'cmd', 'command', or 'script'.", vim.log.levels.ERROR)
					return
				end
				...
				local job_id = vim.fn.jobstart(cmd, {
					...
					env = pre_debug.env,
					on_exit = function(id)
						pre_debug_jobs[id] = nil
					end,
				})

				if job_id <= 0 then
					vim.notify("Failed to start pre_debug command.", vim.log.levels.ERROR)
					return
				end

				pre_debug_jobs[job_id] = true
				vim.defer_fn(function()
					dap.run(launch_spec)
				end, tonumber(pre_debug.wait_ms) or 300)
			end
			_G.BuildSystem = _G.BuildSystem or {}
			_G.BuildSystem.run_dap_spec = run_dap_spec
			...
			local function collect_build_dirs_from_justfile(set, root)
				local justfile =
					vim.fs.find({ "justfile", ".justfile" }, { path = root, upward = false, type = "file" })[1]
						...
						local function step()
							...
							while pos <= out_len and processed < 250 do
								...
								if rel ~= "" and not has_base_ignored_segment(rel) then
									...
									if
										abs
										and not is_in_build_dir(abs, build_dirs)
										and is_relevant_source_file(abs, language)
									then
					...
					while #queue > 0 and processed < 300 do
						...
						if not name then
							...
							if typ == "directory" then
								if
									full_path
									and not base_ignored_dirs[name]
									and not is_in_build_dir(full_path, build_dirs)
								then
									push_dir(full_path)
								end
							elseif typ == "file" then
								if
									full_path
									and not is_in_build_dir(full_path, build_dirs)
									and is_relevant_source_file(full_path, language)
								then
			...
			local function needs_rebuild_for_debug_async(program, language, cb)
				...
				if not root or vim.fn.isdirectory(root) == 0 then
					append_dap_log(
						"rebuild_check_result",
						{ should_build = true, reason = "invalid project root", program = program }
					)
				...
				if not program_abs then
					append_dap_log(
						"rebuild_check_result",
						{ should_build = true, reason = "missing debug program", program = program }
					)
				...
				if not bin_stat then
					append_dap_log(
						"rebuild_check_result",
						{ should_build = true, reason = "missing binary", program = program_abs }
					)
				...
				file_time_bounds_async(root, build_dirs, language, function(bounds)
					...
					if bounds.newest and bounds.newest <= bin_mtime then
						...
						cb(
							false,
							"newest relevant file older/equal than binary: " .. (bounds.newest_path or "unknown"),
							program_abs
						)
						return
					end
					...
					cb(
						true,
						"newest relevant file newer than binary: " .. (bounds.newest_path or "unknown"),
						program_abs
					)
				end)
			end
			...
			_G.BuildSystem = _G.BuildSystem or {}
			_G.BuildSystem.start_debug_session = function()
				ensure_breakpoints_preloaded(start_debug_session)
			end
