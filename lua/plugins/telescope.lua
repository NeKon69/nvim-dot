	...
	config = function()
		...
		local function apply_query(entries, parsed)
			...
			if #all_targets > 0 and result and #result > 0 then
				...
				for _, entry in ipairs(result) do
					...
					if total_score < best_score or (total_score == best_score and time_dist < best_time_dist) then
		...
		local function smart_file_picker()
			...
			local function make_display(entry)
				...
				local should_highlight_time = current_highlight_info.highlight_time
					and (current_highlight_info.target_index == entry.original_index)
			...
			local picker = pickers.new({}, {
				...
				attach_mappings = function(prompt_bufnr, map)
					local function refresh_picker(new_results, move_to_idx, new_highlight_info)
						current_highlight_info = new_highlight_info
							or {
								highlight_index = false,
								highlight_time = false,
								text_patterns = {},
